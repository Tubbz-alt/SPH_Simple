...
int main(int argc, char *argv[])
{
    ...
    int n;
    int fileNum=0;
#pragma acc data copyin(fluid_particles[0:params.number_fluid_particles],boundary_particles[0:params.number_boundary_particles],params)
{
...
double3 computeBoundaryAcceleration(double3 p_pos, double3 k_pos, double3 k_n, double h, double speed_sound)
{
    double3 p_a;
...
void updateAccelerations(fluid_particle *fluid_particles, boundary_particle *boundary_particles, const param *const params)
{
    ...
    for(i=0; i<num_fluid_particles; i++) {
        ...
        for(j=0; j<num_fluid_particles; j++) {
            if (i!=j) {
                double3 q_pos = fluid_particles[j].pos;
                double3 q_v   = fluid_particles[j].v;
                ...
                double3 tmp_a = computeAcceleration(p_pos, p_v, p_density, p_pressure, q_pos, q_v, q_density, q_pressure, params);
    ...
    for(i=0; i<num_fluid_particles; i++) {
        ...
        for (j=0; j<num_boundary_particles; j++) {
            ...
            double3 tmp_a = computeBoundaryAcceleration(p_pos,k_pos,k_n,params->smoothing_radius,params->speed_sound);
            ax += tmp_a.x;
            ay += tmp_a.y;
            az += tmp_a.z;
        }
